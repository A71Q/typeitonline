...
(function($){
    ...
    $.fn.bnKb.kp = function(ev){
        ...
        if (ctrlPressed) 
            ...
            if (opts.showSuggestions.flag && keycode == 9) {
                return $.fn.bnKb.suggestion(this, opts.showSuggestions.div);
            }
            if (opts.showSuggestions.flag && keycode == 37) {
                alert('there');
            }
    ...
    $.fn.bnKb.suggestion = function(myField, div) {
        var result = /\S+$/.exec(myField.value.slice(0, myField.selectionEnd));
        var lastWord = result ? result[0] : null;
        var suggestions = getSuggestion(lastWord);
        var data = "";
        var sLength = suggestions.length;
        for(var i = 0; i < suggestions.length; i++) {
            data += '<a class="suggestion" onkeypress="moveNext(this,'+sLength +' );" id="suggestion_'+ (i + 1) +'" href="#" onclick="insertSuggestion(\'' + suggestions[i] + '\');">' + suggestions[i] + "</a><br>";
        }
        if (sLength <= 0) {
            data = "No Suggestion";
        }
        $("#" + div).html(data);
        $(".suggestion:first-child").focus();
        return false;
    };
