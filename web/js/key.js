function makeEditors(mlayout){var days2save=30;for(var i=0;i<taArray.length;i++){if(mlayout=='bjy'){makeBijoyEditor(taArray[i]);if(i==0){createCookie('kbsetting','bjy',30);switched=false;}}else if(mlayout=='ujy'){makeUnijoyEditor(taArray[i]);if(i==0){createCookie('kbsetting','ujy',30);switched=false;}}else if(mlayout=='pbh'){makeProbhatEditor(taArray[i]);if(i==0){createCookie('kbsetting','pbh',30);switched=false;}}else if(mlayout=='phn'){makePhoneticEditor(taArray[i]);if(i==0){createCookie('kbsetting','phn',30);switched=false;}}else if(mlayout=='eng'){if(i==0){createCookie('kbsetting','eng',30);switched=true;}}}}
function keyfunc(e){var ev=(window.event)?event.keyCode:e.which;var Char=String.fromCharCode(ev);if(e.altKey&&e.ctrlKey&&(Char=='V'||Char=='v')){switched=!switched;}
else if(ev==98){switched=!switched;}
else if(ev==27){switched=!switched;}}
if(document.addEventListener)document.addEventListener('keydown',keyfunc,false);else if(document.attachEvent)document.attachEvent('on'+'keydown',keyfunc);else document['on'+'keydown']=keyfunc;function createCookie(cookieName,cookieData,days){var expiry=new Date();expiry.setTime(expiry.getTime()+days*(24*60*60*1000));if(days!=null){document.cookie=cookieName+"="+escape(cookieData)+"; expires="+expiry.toGMTString()+"; path=/";}else{document.cookie=cookieName+"="+escape(cookieData);}}
function readCookie(cookieName){var exp=new RegExp(escape(cookieName)+"=([^;]+)");if(exp.test(document.cookie+";")){exp.exec(document.cookie+";");return unescape(RegExp.$1);}
else return false;}
function toggleIt(obj){var el=document.getElementById(obj);if(el.style.display=='none'){el.style.display='';}else{el.style.display='none';}}
function hideIt(obj){var el=document.getElementById(obj);el.style.display='none';}
function showIt(obj){var el=document.getElementById(obj);el.style.display='';}
...
{insertAtCursor('\u09C8',1);return false;}
else if(char_e=="o")
...
{return(unijoy[code]);}}
function insertAtCursor(myValue){if(myValue.length==1&&(myValue.charCodeAt(0)=="0"||myValue.charCodeAt(0)=="27")){return;}
lastInserted=myValue;var myField=document.getElementById(activeta);if(document.selection){myField.focus();sel=document.selection.createRange();sel.text=myValue;sel.collapse(true);sel.select();}
else if(myField.selectionStart||myField.selectionStart==0){var startPos=myField.selectionStart;var endPos=myField.selectionEnd;var scrollTop=myField.scrollTop;startPos=(startPos==-1?myField.value.length:startPos);myField.value=myField.value.substring(0,startPos)
+myValue
+myField.value.substring(endPos,myField.value.length);myField.focus();myField.selectionStart=startPos+myValue.length;myField.selectionEnd=startPos+myValue.length;myField.scrollTop=scrollTop;}else{var scrollTop=myField.scrollTop;myField.value+=myValue;myField.focus();myField.scrollTop=scrollTop;}}
...
Array.prototype.clone=function(){var newObj=(this instanceof Array)?[]:{};for(i in this){if(i=='clone')continue;if(this[i]&&typeof this[i]=="object"){newObj[i]=this[i].clone();}else newObj[i]=this[i]}return newObj;};var bijoy=unijoy.clone();bijoy['d']='\u200d'+'\u09BF';bijoy['c']='\u200d'+'\u09C7';bijoy['C']='\u200d'+'\u09C8';function parsebijoy(evnt){var t=document.getElementById(activeta);var e=(window.event)?event.keyCode:evnt.which;if(e=='32'||e=='71')
{formatBijoy();}
if(e=='113')
{if(ctrlPressed){switched=!switched;return true;}}
if(switched)return true;if(ctrlPressed)
...
var char_e=String.fromCharCode(e);if(e==8||e==32)
{carry=" ";old_len=1;return;}
lastcarry=carry;carry+=""+char_e;if(typeof isEnglishOn!='undefined'){if(isEnglishOn=="true"){if(char_e.length!=0&&e!=0){insertAtCursor(char_e);old_len=lastInserted.length;}
return false;}}
bangla=parsebijoyCarry(carry);tempBangla=parsebijoyCarry(char_e);if(tempBangla==".."||bangla=="..")
{return false;}
if(char_e=="g")
{if(carry=="gg")
{insertConjunction('\u09CD'+'\u200c',old_len);old_len=1;return false;}
...
{newChar=bijoy['v']+'\u09CD';insertAtCursor(newChar);old_len=1;return false;}
else if((bangla==""&&tempBangla!=""))
{bangla=tempBangla;if(bangla=="")
{carry="";return;}
else
...
{activeTextAreaInstance=document.getElementById(activeta);text=activeTextAreaInstance.value;malformedKars=[bijoy['c'],bijoy['d'],bijoy['C']];correctKars=['\u09C7','\u09BF','\u09C8'];for($j=0;$j<3;$j++){var malformedKar=malformedKars[$j]
while(karPos=text.indexOf(malformedKar)){if(karPos==-1)break;start=karPos+2;end=karPos+9;for($i=start;$i<end;$i++)
{if(text.substr($i,1)=="\u09CD")continue
else if(text.substr(($i+1),1)=="\u09CD")continue
else
{newtext=text.substr(0,karPos)+text.substr(start,(($i+1)-start))+correctKars[$j]+text.substr(($i+1));text=newtext;break;}}}}
activeTextAreaInstance.value=text;}
function makeBijoyEditor(textAreaId)
{activeTextAreaInstance=document.getElementById(textAreaId);activeTextAreaInstance.onkeypress=parsebijoy;activeTextAreaInstance.onkeydown=checkKeyDown;activeTextAreaInstance.onkeyup=checkKeyUp;activeTextAreaInstance.onblur=formatBijoy;activeTextAreaInstance.onfocus=function(){activeta=textAreaId;};}
